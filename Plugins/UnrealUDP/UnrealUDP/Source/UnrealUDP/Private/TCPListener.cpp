...
bool ATCPListener::StartTCPReceiver(const FString& YourSocketName, const FString& TheIP, const int32 ThePort)
{
	...
	if (!ListenerSocket)
	{
		...
		TimerDel.BindUFunction(this, FName("StartTCPReceiver"), YourSocketName, TheIP, ThePort);
		GetWorldTimerManager().SetTimer(ConnectionTryTimer, TimerDel, 0.1, true);
	...
	GetWorldTimerManager().SetTimer(NewConnectionTimer, this, &ATCPListener::TCPConnectionListener, 0.01, true);
	GetWorldTimerManager().ClearTimer(ConnectionTryTimer);
...
{
	//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("TCPConnectionListener")));

	if (!ListenerSocket)
	{
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("Bad socket")));
		return;
	}

	//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("Good socket")));
	...
	if (ListenerSocket->HasPendingConnection(Pending) && Pending)
	{
		...
		if (ConnectionSocket != NULL)
		{
			...
			GetWorldTimerManager().SetTimer(ListenTimer, this, &ATCPListener::TCPSocketListener, 0.01, true);
		}
	}
}

FString ATCPListener::StringFromBinaryArray(TArray<uint8>& BinaryArray)
{
	BinaryArray.Add(0);
...
{
	//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("TCPSocketListener")));
	...
	while (ConnectionSocket->HasPendingData(Size))
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Read string: %s"), *ReceivedUE4String);
	}
}